...
def find_last_conv_layer(model):
    """Recursively find last Conv2D layer, supports nested Sequential models"""
    for layer in reversed(model.layers):
        if isinstance(layer, tf.keras.layers.Conv2D):
            return layer
        elif isinstance(layer, tf.keras.models.Sequential) or isinstance(layer, tf.keras.Model):
...
def get_gradcam(model, img_array):
    # Call model once to define outputs (fixes Sequential AttributeError)
    _ = model(img_array)
...
if uploaded_file is not None:
    ...
    if st.button("Predict Disease"):
        with st.spinner("Analyzing leaf..."):
            # Call model once for Sequential output fix
            ...
            # Healthy leaf warning if confidence < 70%
